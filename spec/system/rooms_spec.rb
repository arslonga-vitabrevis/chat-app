...
RSpec.describe "チャットルームの削除機能", type: :system do
  ...
  it 'チャットルームを削除すると、関連するメッセージがすべて削除されていること' do
    ...
    # メッセージが削除されているか確認するためにcreate_listを用いて
    # メッセージと紐付いた、ユーザーとチャットルームを中間テーブルに生成
    ...
    # その後、find_linkメソッドで「"チャットを終了する",href:room_path(@room_user.room)」を取得してクリック
    # 「チャットを終了する」をクリックすると、チャットルームは削除されるので
    # 同じタイミングでmessageモデルのカウントが5つ減っていることを確認
    expect{
